...
func cmdLoad(args []string) bool {
	...
	case "SINGLE":
		...
		// Save to DB based on what to load (skip if DB not available)
		if !utils.IsDBAvailable() {
			utils.Config.Log.LogWarning("Database not available (Windows limitation) - skipping data persistence")
			return false
		}
		switch kind {
		case "target":
...
func ShowTargets() {
	if !utils.IsDBAvailable() {
		utils.Config.Log.LogWarning("Database not available - cannot show targets")
		return
	}
...
func ShowHosts() {
	if !utils.IsDBAvailable() {
		utils.Config.Log.LogWarning("Database not available - cannot show hosts")
		return
	}
...
func ShowPorts() {
	if !utils.IsDBAvailable() {
		utils.Config.Log.LogWarning("Database not available - cannot show ports")
		return
	}
	hosts := model.GetAllHosts(utils.Config.DB)
	if len(hosts) == 0 {
